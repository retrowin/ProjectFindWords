...
class StartMenu(QWidget):
    """Отобразить окно для настройки поиска слов.

    Отобразить варианты:
        1. Выбрать количество строк для сетки поиска слов
        2. Настроить игру дополнительно
        3. Начать игру
        4. Выйти из игры
    """
    ...
    def initUI(self):
        self.setWindowTitle("Поиск слов")
...
class CustomizeMenu(QWidget):
    ...
    Отобразить варианты:
        1. Добавить пользовательские слова
        2. Настроить, как генерируются слова
        3. Продолжить, если настройка завершена

    Поднять всплывающие окна, если не выполнены ограничения конфигурации.
    """
...
class App(QWidget):
    """Отобразить окно для основной игры и запустить таймер.

    Отобразить варианты:
        1. Выбрать буквы на таблице
        2. Поставить игру на паузу/Возобновить игру
        3. Выйти из игры

    Атрибуты:
        wordBank: Строка с словами, которые нужно найти в поиске слов.
        wordBankSplit: Список строк с словами, которые нужно найти в поиске слов.
        wordSelected: Строка для хранения текущего выбранного слова.
        xVisited: Список целых чисел для хранения текущих значений строк выбранных букв.
        yVisited: Список целых чисел для хранения текущих значений столбцов выбранных букв.
        inRow: Целое число для определения, находятся ли выбранные буквы подряд.
        progressValue: Целое число для отслеживания найденных слов.
        wordsCompleted: Список строк с найденными словами.
        timeFlag: Флаг времени для отслеживания таймера, если игра была приостановлена или возобновлена.
    """
    ...
    def createTable(self):
        """Сгенерировать таблицу поиска слов."""
        generateAll = False
...
class HighScoreMenu(QWidget):
    """Отобразить окно для отчета о текущих и предыдущих рекордах
